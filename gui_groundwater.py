...
def open_pairwise():
    ...
    desc_top = ctk.CTkLabel(
        ...
        wraplength=520,   # reduced to avoid clipping at right edge
        justify="left",
        anchor="nw",
        font=BODY_FONT
    )
    ...
    desc_bottom = ctk.CTkLabel(
        ...
        wraplength=520,   # reduced to avoid clipping
        justify="left",
        anchor="nw",
        font=BODY_FONT
    )
...
def show_results():
    ...
    if state['CR'] > 0.1:
        ...
        ctk.CTkLabel(main_content, text="Tip: Click a pair to re-evaluate that comparison. Adjust toward the implied value to reduce error.", font=ITALIC_FONT).pack(anchor="w", padx=36, pady=6)
    else:
