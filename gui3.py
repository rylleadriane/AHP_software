# gui2.py — Multi-level AHP GUI for Water Security (refined AHP process)
# Requires: numpy
# Run: python gui2.py
...
domain_progress = {
    d: {
        'status': 'pending',          # pending | done | inconsistent
...
# ------------------------------------------------------------------------------
# Suggestion helper — advisory ranges based on ranking difference (not enforced)
# ------------------------------------------------------------------------------
def suggest_range_for_pair(a, b, ranked_list):
    """
    Provide an advisory Saaty range (broad) for the pair a vs b based on ranking positions.
    This is only a suggestion for the user to consider; it does not restrict choices.
    """
    try:
        pos_a = ranked_list.index(a)
        pos_b = ranked_list.index(b)
    except ValueError:
        return "No ranking guidance available."

    # ranking positions start at 0, so difference = how far apart they are
    diff = abs(pos_a - pos_b)
    # coarse rules:
    # diff 0 -> recommend 1-3 (similar importance)
    # diff 1 -> recommend 2-5 (somewhat different)
    # diff >=2 -> recommend 3-9 (clear difference possible)
    if diff == 0:
        return "Advisory: items ranked similarly — consider 1 to 3 (or 1 if equal)."
    elif diff == 1:
        return "Advisory: adjacent ranks — consider 2 to 5 depending on certainty."
    else:
        return "Advisory: different ranks — consider 3 to 9 for stronger preference."
...
def dashboard_screen():
    ...
    for d in DOMAINS:
        ...
        # View results button if weights exist (so user can view even if inconsistent)
        if dp['weights'] is not None:
    ...
    # If all domains done (CR <= 0.1), allow proceeding to domain-level AHP
    if all(domain_progress[d]['weights'] is not None and domain_progress[d]['CR'] is not None and domain_progress[d]['CR'] <= 0.1 for d in DOMAINS):
        ...
        # show note explaining manual review flow
        tk.Label(frame, text="Note: All domains must have acceptable consistency (CR ≤ 0.1) to proceed to domain-level AHP.",
                 bg="#F9F9F9", font=("Arial", 11, "italic")).pack(pady=(8, 0))
...
def open_pairwise_screen(domain):
    ...
    Also displays an advisory suggestion range based on ranking difference.
    """
    dp = domain_progress[domain]
    ...
    # advisory suggestion
    suggestion = suggest_range_for_pair(a, b, ranked)
    tk.Label(frame, text=f"Suggestion: {suggestion}", bg="#F9F9F9", font=("Arial", 11, "italic")).pack(anchor="w", padx=6, pady=(0, 8))
...
def finalize_domain_ahp(domain):
    ...
    and set status depending on CR. If CR <= 0.1, status is 'done', otherwise 'inconsistent'.
    The user may manually revisit inconsistent pairs from the results screen.
    """
    dp = domain_progress[domain]
    ranked = dp['ranked_params']
    ...
    dp['CR'] = CR
    dp['matrix'] = A
    # set status based on CR — user must decide to revisit if inconsistent
    if CR <= 0.1:
        dp['status'] = 'done'
    else:
        dp['status'] = 'inconsistent'

    # show results screen so user can inspect and optionally revisit inconsistent pairs
...
def show_domain_results(domain):
    ...
    show the top inconsistent pairs as clickable buttons allowing the user to edit that pair.
    """
    dp = domain_progress[domain]
    clear_screen()
    ...
    if dp['weights'] is None:
        tk.Label(frame, text="No results yet. Complete pairwise comparisons first.", bg="#F9F9F9", font=BODY_FONT).pack(anchor="w")
    ...
    status_text = "ACCEPTABLE" if dp['CR'] <= 0.1 else "INCONSISTENT — consider revising pairs"
    tk.Label(frame, text=f"Consistency Ratio: {dp['CR']:.4f}   ({status_text})", bg="#F9F9F9",
    ...
    if dp['CR'] > 0.1:
        ...
        tk.Label(frame, text="Tip: Click a pair to re-evaluate that comparison. Recompute until CR ≤ 0.1.", bg="#F9F9F9",
                 font=("Arial", 11, "italic")).pack(anchor="w", padx=36, pady=(6, 8))
...
def open_domain_level_pairwise():
    ...
    # advisory suggestion for domain-level pair
    suggestion = suggest_range_for_pair(a, b, ranked)
    tk.Label(frame, text=f"Suggestion: {suggestion}", bg="#F9F9F9", font=("Arial", 11, "italic")).pack(anchor="w", padx=8, pady=(0, 8))
...
def finalize_domain_level():
    ...
    # do not auto-mark anything here; just show results (user flow remains manual)
...
def show_domain_level_results():
    ...
    status_text = "ACCEPTABLE" if domain_level_result['CR'] <= 0.1 else "INCONSISTENT"
    tk.Label(frame, text=f"Consistency Ratio: {domain_level_result['CR']:.4f}   ({status_text})",
...
def save_as_csv(path):
    ...
    try:
        with open(path, "w", newline='', encoding="utf-8") as f:
            ...
            for d in DOMAINS:
                dp = domain_progress[d]
                cr_val = f"{dp['CR']:.4f}" if dp['CR'] is not None else ""
